...
public class Client {
	...
	public static void main(String[] args) {
		...
		try {
			...
			GetSearcher comp = (GetSearcher) registry.lookup(name);			// look up the remote object by name
			
			int howMany = (Integer.parseInt(args[1]));						// accept user input for the number of searches
			
			// call a server object and run the searchBenchmark method
			Map<Node, Map<Node, Integer>> message = (Map<Node, Map<Node, Integer>>) comp.getSearcher().searchBenchmark(howMany, nodes);
